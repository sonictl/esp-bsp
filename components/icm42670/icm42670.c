...
static esp_err_t icm42670_set_i2c_interface(icm42670_handle_t sensor);
...
icm42670_handle_t icm42670_create(i2c_port_t port, const uint8_t dev_addr)
{
    ...
    icm42670_set_i2c_interface(sensor);
...
esp_err_t icm42670_config(icm42670_handle_t sensor, const icm42670_cfg_t *config)
{
    ...
    /* Gyroscope */
    data[0] = ((config->gyro_fs & 0x03) << 4) | (config->gyro_odr & 0x0F);
    /* Accelerometer */
    data[1] = ((config->acce_fs & 0x03) << 4) | (config->acce_odr & 0x0F);
...
esp_err_t icm42670_get_acce_sensitivity(icm42670_handle_t sensor, double *sensitivity)
{
    esp_err_t ret = ESP_FAIL;
    uint8_t acce_fs;
...
esp_err_t icm42670_get_gyro_sensitivity(icm42670_handle_t sensor, double *sensitivity)
{
    esp_err_t ret = ESP_FAIL;
    uint8_t gyro_fs;
...
esp_err_t icm42670_get_temp_value(icm42670_handle_t sensor, double *value)
{
    esp_err_t ret;
    uint16_t raw_value;
...
static esp_err_t icm42670_set_i2c_interface(icm42670_handle_t sensor)
{
    ...
    return icm42670_write(sensor, ICM42670_INTF_CONFIG1, data, sizeof(data));
}
...
static esp_err_t icm42670_read(icm42670_handle_t sensor, const uint8_t reg_start_addr, uint8_t *data_buf, const uint8_t data_len)
{
    ...
    ESP_LOGW(TAG, "I2C(%d) read addr: 0x%02x reg: 0x%02x", sens->bus, sens->dev_addr, reg_start_addr);
